...
async function main() {
  ...
  const processTransaction = (
    transactions: string[],
    createFn: (transaction: string) => Promise<void>,
  ) => {
  ...
  for (let i = 0; i < batchesFormance.length; i++) {
    b1.update({ value: i });
    await Promise.all([
      processTransaction(batchesFormance[i], createTransaction),
      processTransaction(batchesMysql[i], createMysqlTransaction),
    ]);
  }
